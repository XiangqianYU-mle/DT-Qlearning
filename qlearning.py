...
def read_file():
    ...
    with open(path + './data/transitions.csv', 'r') as csvfile:
    ...
    with open(path + './data/rewards_0.4g.csv', 'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
...
bst['(0 1)'] = pi['(0 1)']
new_state = '(0 1)'
while(pi[new_state] != 'Exit'):  
    new_state = next_state(pi[new_state], new_state)
    bst[new_state] = pi[new_state]
